...
public class TwoSum {
    ...
    public int[] twoSum(int[] nums, int target) {
        ...
        Integer placeHolder = new Integer(-1);
        Integer tmp = null;
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            tmp = map.get(pal);
            if (tmp != null && tmp != placeHolder) { // tmp != placeHolder is used to process the same number appears
                                                     // and we know the pal got is not what we want
                System.out.println("Numbers are " + pal + " and " + nums[i]);
                return new int[] { tmp, i + 1 };
            }

            // below is the avoid the number to the end will override the index of the same number ahead
            if (!map.containsKey(nums[i])) {
                map.put(nums[i], i + 1);
            }

            // below is for the case when the value of pal and target are the same
            if (pal != nums[i]) {
                map.put(pal, placeHolder);
            }
        }
