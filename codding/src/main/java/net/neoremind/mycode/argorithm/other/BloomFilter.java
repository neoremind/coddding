 ...
 * <ul>
 * <li></li>
 * <li>1. 布隆过滤器实际上是一个很长的二进制向量和一系列随机映射函数</li>
 * <li>2. False postive（误判）</li>
 * <li>3. 100%召回率,确保“definitely no”从而容忍“probably yes”</li>
 * <li>4. 不允许元素的删除</li>
 * </ul>
 * <p/>
 ...
 * <p/>
 * bloom filter的这种位图模式带来两个问题：一个是误报（false positives），在查询时能提供“一定不存在”，但只能提供“可能存在”，因为存在其它元素被映射到部分相同bit位上，导致该位置1
 * ，那么一个不存在的元素可能会被误报成存在；另一个是漏报（false nagatives），同样道理，如果删除了某个元素，导致该映射bit位被置0，那么本来存在的元素会被漏报成不存在。由于后者问题严重得多，所以bloom
 * filter必须确保“definitely no”从而容忍“probably yes”，不允许元素的删除。
 * <p/>
